...
let probabilidadDeSacarSeis : number = 0

while(numDado != 0){
    numDado = rls.questionInt("ingrese la cantidad de dados: ")
  denominador = Math.pow(caraDado, numDado)
  probabilidadDeSacarSeis = 1/denominador
  console.log(`La probabilidad de todos 6 es 1/${denominador} y ${probabilidadDeSacarSeis * 100}%`)
} 
